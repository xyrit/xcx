...
{			
			...
			public function get_xcx_token($uid=0){
						
							//查看store_id
							$appid = db::name('dc_appid')->where('uid',$uid)->find();
							
							$token1 = db::name('config')->where('name','xcx_access_token')->where('type',$uid)->find();
			    			...
			    			if(empty($token1) || $token1['add_time']+7200<$time){
			    						...
			    						// var_dump($token);
			    						$token = json_decode($token,true)['access_token'];
			    						$token1?db::name('config')->where('name','xcx_access_token')->where('type',$uid)->update(['value'=>$token,'add_time'=>$time]):db::name('config')->insert(['name'=>'xcx_access_token','value'=>$token,'add_time'=>$time,'type'=>$uid]);
			...
			public function moban($type,$order_id){
						...
						$appid = db::name('dc_appid')->where('uid',$pid)->find();
						...
						$token  = $this->get_xcx_token($appid['uid']);
						$param = [];
						$param['touser'] = $openid;
