...
class Pay extends Model
{
	     ...
	     * 店铺信息
	     */
	    public function jnms_micropay($data,$type){
			    $param['merNo']=$data['merchant_id'];
		        //付款码
		        $param['authCode']=$data['authCode'];
		        //商户订单号
		        $param['orderNo']=$data['order_sn'];
		        //支付渠道
		        $param['channelFlag']=$type==1?'00':'01';//00微信01支付宝
		        //订单金额
		        $param['amount']=$data['price']*100;//以分为单位
		        //请求交易的流水号
		        
		        $param['reqId']=$data['order_sn'];
		        //请求时间
		        $param['reqTime']=date("YmdHis");
		        $param['goodsName']='订单号:'.$data['order_sn'];
		       //	$param['signIn'] = $this->sign($param,$data['key']);
		         $url="http://scp.yufu99.com/scanpay-api/api/unScanCodePay20";
      			 $result = $this->httpRequst($url,$param);
      			 $result = json_decode($result,true);
      			 add_log($result);
      			 if(isset($result['result'])&&$result['result'] == '0000'){
      			 		p($result);
      			 }else{
