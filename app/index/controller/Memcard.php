...
class Memcard extends Home
{
    ...
    public function my_two(){
          ($store_id = input('store_id')) || err('store_id is empty');
        $Memcard = model('ScreenMemcardUse');
        $data = $Memcard->lists_two($store_id,UID);
        succ($data);
    }
    public function detail(){
        ($store_id = input('store_id')) || err('store_id is empty');
        ($card_code = input('card_code')) || err('card_code is empty');
        $Memcard = model('ScreenMemcardUse');
        $data = $Memcard->detail($store_id,UID,$card_code);
        succ($data);
    }
  	public function add(){
  				$id = input('id');
