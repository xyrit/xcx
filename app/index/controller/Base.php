...
{	
	...
	public function index()
	{
		...
		$data = model('Merchants')->lists($lon,$lat,1000,input('area'),input('admin_id',0),$page,$trade,$search_name);
		succ($data);
	}
	...
	//手机号登录注册
	public function login1($phone='',$password='',$code=''){
	...
	public function quick_login(){
				...
				($code = input('code')) || $this->err('code is empty');
				...
				switch($type){
					...
					$data = curl_post($url,['appid'=>$appid,'secret'=>$secret,'js_code'=>$code,'grant_type'=>'authorization_code']);
					...
					if($encryptedData&&$iv){
							...
							isset($unionId['unionId'])&&($user_info['unionid'] =$unionId['unionId']);
					}
					$user_info['userid'] = $admin_id;
					$user_info['memimg']  = input('picture','');
					$user_info['nickname'] = input('user_name','');
					//查询是否存在openid
					$user_info['type'] = $type;
					// dump($user_info);
					$token = model('User')->quick_reg($openid,$user_info,$admin_id);
					succ($token);
					break;
					case 'dc':
					$url = 'https://api.weixin.qq.com/sns/jscode2session?=APPID&secret=SECRET&js_code=JSCODE&=';
					$data = curl_post($url,['appid'=>'wx6480c903c36e1f18','secret'=>'d8d611a3e98b3c4fead347bdfcbb2ead','js_code'=>$code,'grant_type'=>'authorization_code']);
					$data = json_decode($data,true);
					if(isset($data['errcode'])){
							err($data['errmsg']);
					}
					$openid = $data['openid'];
					$sessionKey = $data['session_key'];
					$token = model('User')->quick_reg($openid,['type'=>$type]);
					succ($token);
					break;
				}
	}

	public function quick_login2(){
				($type = input('type')) || $this->err('type is Empty');
				($code = input('code')) || $this->err('code is empty');
...
//		var_dump($data);
	...
	public function distance_store(){
		...
		$lat = input('lat',0);
		$page = input('page',0);
		$data = model('Merchants')->distance_store($lon,$lat,1000,input('area'),input('admin_id',0),$page);
		succ($data);
	}

	public function distance_store1(){
	
		$lon = input('lon',0);
		$lat = input('lat',0);
	
		$data = model('Merchants')->distance_store1($lon,$lat,1000,input('area'),input('admin_id',0));
		succ($data);
	}

	public function distance_store2(){
	
		$lon = input('lon',0);
		$lat = input('lat',0);
		$page = input('page',0);
		$data = model('Merchants')->distance_store2($lon,$lat,1000,input('area'),input('admin_id',0),$page);
		succ($data);
	}
	public function wx(){
