...
class Order extends Model
{
		...
		public function info_detail($order_id,$uid,$field=true){
				...
				foreach($goods as $key => &$vv){
						$picture = $vv['goods_img'];
						if(preg_match("/\x20*https?\:\/\/.*/i",$vv['goods_img'])){
						    $vv['goods_img'] = $picture;
						}else{
						    $vv['goods_img'] = URL.$picture;
						}
				}
				$order->goods = $goods; 
				return $order;
		...
		//添加订单
		public function add($goods_id,$nums_id,$attr_id,$address_id,$store_id,$coupons_id,$staff_id){
				//商品id
...
//				}
				...
				//减去优惠权，积分，加上邮费
				$order['order_benefit'] = $order['integral_money'] + $order['coupon_price'];
				$order['total_amount'] = $order['total_amount']+$order['shipping_price'];
		...
		//添加订单
		public function add_new($goods_id,$nums_id,$attr_id,$address_id,$store_id,$coupons_id,$staff_id,$shipping_ps,$discount_price,$integral_price,$balance_price,$discount,$card_code,$yh_integral){
				//商品id
				$goods_id = explode(',',$goods_id);
				$attr_id = explode(',',$attr_id);
				$nums = explode(',',$nums_id);
				...
				foreach($goods_id as $key =>$v){
						...
						if($goods['put_xcx']!=2){
										return $this->error($goods['goods_name'].'已下架');	
						}
						if($goods['is_sku']){
								$goods_sku = db::name('goods_sku')->where('goods_id',$v)->lock(true)->where('sku_id',$attr_id[$key])->find();
								//判断库粗是否足够
...
//				}
				...
				if ($card_code) {
					$card_code = $card_code;
					$card_price =$integral_price;
					$card_balance = $yh_integral;
				}else{
					$Memcard = model('ScreenMemcardUse')->lists($store_id,UID);
					$card_balance = $card_price = $card_code = 0;
					$price = $order['order_amount'];
					if($Memcard&&$price>=$Memcard[0]['credits_discount']){
						$Memcard = $Memcard[0];
						if($integral_price>'0.00'){
							while(($Memcard['card_balance']-=$Memcard['credits_use'])>=0&&($price-=$Memcard['credits_discount'])>=0&&$card_price<$integral_price){
											$card_price+=$Memcard['credits_discount'];
											$card_balance+=$Memcard['credits_use'];
							}
						}
				...
				$order['discount_money'] = $discount_price;
				$order['user_money'] = $balance_price;
				$order['coupon_code'] = isset($Coupons)?$Coupons['usercard']:'';
				$order['order_sn'] = $this->get_order_sn();
				$order['mid'] = UID;
				$order['user_id'] = $store_id;
				...
				$order['area_id']  =$address['area_id'];
				//减去优惠权，积分，加上配送费
				$order['dc_ps_price'] = $shipping_ps; //配送费
				$order['order_benefit'] = $order['integral_money'] + $order['coupon_price'] + $discount_price;
				$order['total_amount'] = $order['total_amount']+$shipping_ps;
				$order['order_amount'] = $order['order_amount']-$card_price-$balance_price - $discount_price+$shipping_ps;
				$order['order_status'] = 1;
				$order['paystyle'] = 0;
		...
		public function create_sign($order_id,$type){
					...
					if ($order->user_money>0) {
						...
						$memcard_id =db::name('screen_memcard_use')->where('card_code',$order->card_code)->value('memcard_id');
						$cart_id1 = db::name('screen_memcard')->where('id',$memcard_id)->value('card_id');
						if(empty($cart_id1)){
						...
						if($order->user_money>db::name('screen_memcard_use')->where('unionid',$unionid1)->where('card_id',$cart_id1)->value('yue')){
								return $this->error('余额不足');					
						}
					}
					//查询商品信息
					...
					($cate = db::name('merchants_cate')->where(array('merchant_id'=>$mid,'checker_id'=>0,'status'=>1))->where($bank)->find()) || $this->error('没有开通支付功能');
					// echo db::name('merchants_cate')->getLastSql();
					...
					switch($cate['wx_bank']){
						case '3':
						// file_put_contents('bank.txt','微信官方');
...
//				      	$prepay_id = explode('=',$data->package)[1];
			        	...
			        	//接口类型
					        ...
					        $param['total_fee'] = (int)$price;
				     		//p($param);
				      		$param['notify_url']='https://mp.youngport.com.cn/index/Notify/xy';
...
-----END RSA PRIVATE KEY-----';
				        ...
				        case '10';
					        ...
					        $param['total_fee'] = (int)$price;
				     		//p($param);
				      		$param['notify_url']='https://mp.youngport.com.cn/index/Notify/pf';
	...
	private function httpRequst($url,$data,$res,$appkey){
		...
		public function pay($out_trade_no,$transaction_id,$price,$bank_id=0,$paystyle = 1){
    				//查询订单是否支付
    				if(!$order = $this->where(array('order_sn'=>$out_trade_no,'type'=>1))->field('order_id,order_amount,mid,user_id,coupon_code,order_status,mid,card_code,integral,mobile,add_time,staff_id,user_money')->find()){
    								err('没有找到订单');
    				}
    				if($order->order_status!==1){
    						err('已经支付了');
    				}
    				// file_put_contents('wx6.txt',$order->order_id);
    				//开启事务
    				$this->startTrans();
    				//修改订单状态
    				$order->pay_time = time();
    				...
    				$memcard_id =db::name('screen_memcard_use')->where('card_code',$order->card_code)->value('memcard_id');
					$card = db::name('screen_memcard')->where('id',$memcard_id)->select();
    				//获得积分
    				if($card){
    						$card = $card[0];
    				...
    				if($order->card_code){
    					if($order->integral||$order->user_money){
    						...
    						$ScreenMemcardUse->updateuser($order->card_code,-$order->integral,$out_trade_no,$order->user_money);
    					}
    				}
					...
					if($order->coupon_code){
						...
						if($coupons){
		    						...
		    						add_log($status);
		    						$coupon_result = json_decode($status,true);
		    						file_put_contents('./data/log/xcx/'.date("Y_m_").'card_coupon.log', date("Y-m-d H:i:s") . ',消费使用，优惠券code:'.$order->coupon_code.',核销结果:' . json_encode($coupon_result) . PHP_EOL, FILE_APPEND | LOCK_EX);
						}
    				...
    				if(!db::name('pay')->where('order_id',$order->order_id)->find()){
    							...
    							$pay['status'] = 1;
    							$pay['bank'] = $bank_id;
    							if($order->staff_id)$pay['checker_id'] = $order->staff_id;
    							//先获得cate_id
		    		...
		    		foreach($order_goods as $v){
		    					...
		    					if($v['spec_key']){
		    							if(!db::name('goods_sku')->where('sku_id',$v['spec_key'])->setInc('quantity',$v['goods_num'])){
		    								if (db::name('goods_sku')->where('sku_id',$v['spec_key'])->find()) {
		    									$this->rollback();
		    									return $this->error('修改商品库存失败');
		    								}
		    							}else{
