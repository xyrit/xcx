...
class UnifiedOrder_pub extends Wxpay_client_pub
{
    ...
    function getPrepayId()
    {
        $this->postXml();
        $this->result = $this->xmlToArray($this->response);
        file_put_contents('./data/log/szlz/' . date("Y_m_") . 'wxpay_wxjs.log', date("Y-m-d H:i:s") . 'result：' . json_encode($this->result) . PHP_EOL, FILE_APPEND | LOCK_EX);
...
class WxPayMicroPay extends Wxpay_client_pub
{
    ...
    public function pay()
    {
        ...
        $this->result = $this->xmlToArray($this->response);
        $result = $this->result;
        file_put_contents('./data/log/szlz/' . date("Y_m_") . 'wxpay_micro.log', date("Y-m-d H:i:s") . '失败：' . json_encode($result) . PHP_EOL, FILE_APPEND | LOCK_EX);
//签名验证
//        $sign = $result['sign'];
//        unset($result['sign']);
        ...
        while ($queryTimes > 0) {
            ...
            if ($succResult == 2) {
                ...
                continue;
             	...
             	return $queryResult['transaction_id'];
            } else {//订单交易失败
                return [false,"订单交易失败"];
            }
        }
...
//            throw new WxpayException("撤销单失败！");
    ...
    {	
    	$sub_mch_id = $this->parameters['sub_mch_id'];
    	$this->parameters = null;
        $this->parameters["out_trade_no"] = $out_trade_no;
        $this->parameters['sub_mch_id'] = $sub_mch_id;
